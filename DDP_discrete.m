...
for j = (Horizon-1):-1:1
    ...
    Ox = A(:,:,j)'* Vx(:,j+1) + Lx(:,j);
    Ou = B(:,:,j)' * Vx(:,j+1) + Lu(:,j);
    Oxx = A(:,:,j)' * Vxx(:,:,j+1) * A(:,:,j) + Lxx(:,:,j);
    Ouu = B(:,:,j)' * Vxx(:,:,j+1) * B(:,:,j) + Luu(:,:,j);
    Oxu = A(:,:,j)' * Vxx(:,:,j+1) * B(:,:,j) + Lux(:,:,j)';   
    Oux = Oxu';
    ...
    V(j) = V(j+1) + l_k(:,j)'* Ou + 0.5 * l_k(:,j)' * Ouu * l_k(:,j);
    Vx(:,j) = Ox + L_k(:,:,j)' * Ou + Oxu * l_k(:,j) + L_k(:,:,j)' * Ouu * l_k(:, j);
    Vxx(:,:,j) = Oxx + L_k(:,:,j)' * Oux + Oxu * L_k(:,:,j) + L_k(:,:,j)' * Ouu * L_k(:,:,j);
